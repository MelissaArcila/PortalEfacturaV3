...
|-------|-------------------------------|-------------------|
| Como **Usuario** | Quiero autenticarme con mi correo electrónico y contraseña | Para acceder de forma segura al Portal Unificado |
...
| 4 | Credenciales incorrectas | El usuario no ha alcanzado el límite de 5 intentos fallidos | El usuario ingresa un correo electrónico que no existe en el sistema o una contraseña incorrecta y presiona el botón para ingresar | El sistema muestra el mensaje "Credenciales incorrectas", no revela si el error fue en el correo electrónico o en la contraseña, incrementa el contador de intentos fallidos para ese usuario y registra el evento fallido en auditoría con fecha/hora, IP local, IP pública y correo electrónico ingresado | ☐ | ☐ | ☐ |
| 5 | Bloqueo de cuenta por intentos fallidos | El usuario ha ingresado credenciales incorrectas en 5 intentos consecutivos | El usuario intenta autenticarse por quinta vez con credenciales incorrectas | El sistema bloquea la cuenta del usuario, muestra el mensaje "Credenciales incorrectas" sin revelar que la cuenta ha sido bloqueada, registra el evento de bloqueo en auditoría con fecha/hora, IP local, IP pública y correo electrónico, e inicia un temporizador de 30 minutos para desbloqueo automático | ☐ | ☐ | ☐ |
...
| 27 | Consulta de registros de auditoría para investigación de incidentes | Se necesita investigar un incidente de seguridad o problema de acceso | Un usuario autorizado consulta los registros de auditoría | El sistema debe permitir consultar los registros de auditoría filtrando por: correo electrónico, tipo de evento, rango de fechas, IP origen, resultado (exitoso/fallido), nivel de severidad, cliente (NIT o nombre), permitiendo identificar patrones de comportamiento y reconstruir la secuencia completa de eventos | ☐ | ☐ | ☐ |
